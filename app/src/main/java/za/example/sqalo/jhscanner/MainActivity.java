...
//instance.getIntent();
        ...
        if (Build.VERSION.SDK_INT > 17) {
            ...
            this.navigationView.setVisibility(View.VISIBLE);
        } else {
        ...
        try {
            if (this.bundle != null && !this.bundle.getString("barcode").equals("")) {
                this.status = this.bundle.getString("barcode");
                if (!this.status.equals("barcodefault")) {
                    this.edtbarcode.setText(this.status);
                }
                this.edtuseremail.setText(this.bundle.getString("username"));
                this.edtpass.setText(this.bundle.getString("password"));
                FillHiddenData();
                new DoLogin().execute(new String[]{""});
                SharedPreferences.Editor editor = getSharedPreferences("MyApp_Settings", 0).edit();
                if (!this.edthidenuserid.getText().toString().equals("")) {
                    editor.clear();
                    editor.putString("keycouserid", this.edthidenuserid.getText().toString());
                } else {
                    editor.clear();
                    editor.putString("keyassistuserid", this.edthidenuserrole.getText().toString());
                }
                editor.commit();
            }
        } catch (Exception ex) {
        ...
        this.btnlogin.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                MainActivity.this.FillHiddenData();
                new DoLogin().execute(new String[]{""});
                SharedPreferences.Editor editor = MainActivity.this.getSharedPreferences("MyApp_Settings", 0).edit();
                if (!MainActivity.this.edthidenuserid.getText().toString().equals("")) {
                    editor.clear();
                    editor.putString("keycouserid", MainActivity.this.edthidenuserid.getText().toString());
                } else {
                    editor.clear();
                    editor.putString("keyassistuserid", MainActivity.this.edthidenuserrole.getText().toString());
                }
                editor.commit();
            }
        });
