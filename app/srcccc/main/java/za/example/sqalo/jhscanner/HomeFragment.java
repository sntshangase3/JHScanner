...
//Assistant click to premium hom
    ...
    public void ClearOldGrorequest() {
        ...
        try
        {
                ...
                //========
                String querya = "select * from [UserProductShoppingList] where [userid]=" + activity.edthidenuserid.getText().toString();
                PreparedStatement psa = con.prepareStatement(querya);
                ResultSet rsa = psa.executeQuery();

                while (rsa.next()) {
                    //  Log.d("Remindersaervice In", "Co-user");
                    Date bestbefore = null;
                    Date today = new Date();
                    SimpleDateFormat date_format = new SimpleDateFormat("yyyy-MM-dd");
                    String todaydate = date_format.format(today);
                    try {
                        Date date = new Date(date_format.parse(rsa.getString("purchasedate").toString()).getTime());
                    ...
                    long daysleft = getDateDiff( bestbefore,today, TimeUnit.DAYS);
                    Log.d("Remindersaervice In", "Days"+daysleft);
                    if (daysleft >= 2) {

                        //##Update isNotified/Read
                        String commands = "delete from [UserProductShoppingList]  where [id]='" + rsa.getString("id").toString() + "'";
    ...
    public void NoticeWeekly_ProductDaysUpdate() {
        ...
        try {
                ...
                if (!activity.edthidenuserid.getText().toString().equals("")) {
                 /* String query1 = "select [id],[bestbefore] from [UserProduct] where [userid]=" + activity.edthidenuserid.getText().toString();
                   ...
                   // ps.setQueryTimeout(120*1000);
                    ...
                    rs11.next();
/*
                   String query1 = "select [id],[bestbefore] from [UserProduct] where [userid]=" + rs11.getString("userid").toString();
                    PreparedStatement ps1 = con.prepareStatement(query1);
                    ResultSet rs1 = ps1.executeQuery();
                   // ps1.setQueryTimeout(120*1000);
                    String dates = new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH).format(Calendar.getInstance().getTime());
        ...
        try {
            ...
            if (con == null) {
                ...
                if (activity.edthidenuserid.getText().toString().equals("")) {
                    ...
                    String query = "select * from [UserProductNotification] where [userid]='" + activity.edthidenuserid.getText().toString() + "' and  [isread]='No' order by [categoryid] asc";
                    PreparedStatement ps = con.prepareStatement(query);
                    ResultSet rs = ps.executeQuery();
                    while (rs.next()) {
                        totalnoticeonceoff = totalnoticeonceoff + 1;
